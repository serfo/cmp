...
class Settings {
    ...
    load() {
        try {
            const saved = localStorage.getItem('zabbixSettings');
    ...
    save() {
        try {
            localStorage.setItem('zabbixSettings', JSON.stringify(this.data));
        } catch (e) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof echarts !== 'undefined') {
        ...
        clearTimeout(window.echartsTimeout);
        console.log('ECharts 已加载');
    } else {
        // 监听 CDN 加载完成事件
        window.addEventListener('echartsLoaded', function() {
            console.log('ECharts CDN 加载完成');
            // 可以在这里触发依赖 ECharts 的初始化
        });
    }
});
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // 确保 settingsManager 已经初始化（防御性检查）
    if (typeof Settings !== 'undefined') {
        if (!window.settingsManager) {
            window.settingsManager = new Settings();
        }
    } else {
        // 如果 Settings 类不存在，创建一个空的替代对象
        console.warn('Settings class not found, creating placeholder');
        window.settingsManager = {
            showDialog: function() { console.log('Settings dialog not available'); }
        };
    }
